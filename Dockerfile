# Standalone InvoiceNinja MCP server, run as a sidecar next to InvoiceNinja.
#
# Multi-stage: resolve dependencies into a venv with uv in the builder, then
# ship only the venv + app on a plain python-slim runtime. This keeps the
...
COPY app/server.py app/invoiceninja-api-docs.yaml ./
